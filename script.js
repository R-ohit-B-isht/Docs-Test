...
async function mergeDocumentation() {
  ...
  const repoDir = path.join(docsDir, repoName, 'docs'); // Update the repoDir path

  // console.log(`Cloning ${repo}...`);
  // Clone the repository
  try {
    await exec(`git clone ${repo} "${repoDir}" `);
    console.log(`git clone ${repo} "${repoDir}" `);
    console.log(`Cloned ${repo} successfully in ${repoDir}.`);
  } catch (error) {
